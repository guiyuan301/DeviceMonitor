...
public:
    // 启动时从数据库回灌历史数据(仅填充内存, 不触发告警信号):
    // 告警/事件记录会重新编号衔接 m_nextAlarmId, 抓拍索引恢复到内存
    void seedAlarms(const QList<AlarmItem> &items);
    void seedSnapshots(const QList<SnapItem> &items);
...
private:
    ...
    enum {
        ...
        kMaxSnaps   = 30,     // 精简模式: 抓拍索引少留一些
#else
        kMaxSnaps   = 100,
#endif
        kSnapEveryN = 10000,     // 每 N 件抽检抓拍一次
        kTempWarn   = 35,     // °C
        kTempCrit   = 45,
        kHumiWarn   = 70,     // %RH
        kHumiCrit   = 85,
