...
数据中枢 `DataManager`（`src/core/datamanager.h`）是唯一对接面，服务端解析线程按
**项目文档-V3 §五** 调用即可：

```cpp
DataManager::instance().onDeviceData(d);            // 每条 0x01 数据上报(含产量)
DataManager::instance().setDeviceOnline(id, false); // 心跳超时
DataManager::instance().addSnapshot(id, jpgPath, "抽检·第120件"); // 0x12 抓拍收齐
DataManager::instance().muteBuzzer(-1, 300);        // HMI 消音 → 服务端转 0x03 下发
// 抓拍请求(抽检里程碑/告警留档/手动) → 服务端转 0x04 命令给采集板:
connect(&DataManager::instance(), &DataManager::snapshotRequested,
        this, [](int devid, const QString &reason) { /* server_send_cmd(...) */ });
```

FFmpeg 拉流命令在视频页输入框配置（QSettings 持久化）；板上板C 推流命令见项目文档 §2.5/§6.2。
